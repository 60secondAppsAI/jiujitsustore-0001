...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Inventorys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalInventorys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInventorys">
                <template slot="header">
  ...
  <base-input label="LastUpdated" type="text" placeholder="Enter LastUpdated" v-model="inventoryToAdd.lastUpdated"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="inventorys" :row-key="record => record.InventoryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InventoryPictureView :inventorys="inventorys" />
          </div>
...
import InventoryPictureView from './InventoryPictureView.vue';


const inventorysColumns = [
  "inventoryId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    InventoryPictureView
  },

  data() {
    return {
      modalInventorys: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'inventoryId',
          visible: true,
          scopedSlots: { customRender: 'inventoryId' },
          sorter: true
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} inventorys`,
      },
      ...
      sortBy: 'inventoryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInventorysTable() {
      ...
      let inventorysTableData = [];
      for (let i = 0; i < this.inventorys.length; i++) {
        inventorysTableData.push({
          id: i,
          inventoryId: this.inventorys[i].inventoryId,
          year: this.inventorys[i].year,
          date: this.inventorys[i].date,
          competitionId: this.inventorys[i].competitionId,
          inventoryId: this.inventorys[i].inventoryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-inventorys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInventorysChanged', this.searchQuery);
		//this.renderInventorysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalInventorys = false;

      const currentDate = new Date().getTime();
      this.inventoryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.inventoryToAdd);
      console.log(jsonData);
      
      const res = await InventoryService.addInventory(jsonData);
