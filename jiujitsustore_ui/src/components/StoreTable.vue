...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalStores = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalStores">
                <template slot="header">
  ...
  <base-input label="Location" type="text" placeholder="Enter Location" v-model="storeToAdd.location"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="stores" :row-key="record => record.StoreId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <StorePictureView :stores="stores" />
          </div>
...
import StorePictureView from './StorePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      sortBy: 'storeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderStoresTable() {
      ...
      for (let i = 0; i < this.stores.length; i++) {
        storesTableData.push({
          ...
          year: this.stores[i].year,
          date: this.stores[i].date,
          competitionId: this.stores[i].competitionId,
          storeId: this.stores[i].storeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-stores',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForStoresChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.storeToAdd.created = currentDate;
