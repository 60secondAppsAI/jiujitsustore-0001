					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateInventory()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Inventorys = true">Add</base-button>
								              <modal :show.sync="modal2Inventorys">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Inventory Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="InventoryId" type="text" placeholder="Enter InventoryId" v-model="inventoryDetails.inventoryId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Quantity</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Quantity" type="text" placeholder="Enter Quantity" v-model="inventoryDetails.quantity"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Last Updated</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="inventoryDetails.lastUpdated"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateInventory() {
      ...
      const jsonData = JSON.stringify(this.inventoryDetails);
      const res = await InventoryService.update(jsonData);
...
//        this.getAllInventorys();
    ...
    async getInventoryDetails() {
      try {
			let response = await InventoryService.get(this.inventoryId);
